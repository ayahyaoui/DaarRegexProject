...
public static void main(String arg[]){
    ...
    if (arg.length == 2) {
      ...
      regEx = scanner.next();
      */
      regEx = "a|bc*";//"S(a|g|r)*con";
      pathText = System.getProperty("user.dir") + "/regex/RegEx.java";
      System.out.println("Working Directory = " + pathText);
      try {
    ...
    System.out.println("START" + arg.length);
    ret = TeacherParsing(regEx);
    if (ret == null){
      System.out.println("Bad regex");
      return;
    }
    automaton = ConvertAutomaton(ret);
    if (automaton != null)
    {
      try {
  ...
  /*
   * Just moving teacher code to clarify main function
   */
  public static RegExTree TeacherParsing(String Regex)
  {
    RegExTree ret = null;
  ...
  private static FinalAutomaton ConvertAutomaton(RegExTree ret) {
 ...
 */
 ...
 //FROM TREE TO PARENTHESIS
  ...
  public String toString(int deph) {
    ...
    if (subTrees.isEmpty()) return rootToString();
    String result = rootToString() + " " + openBracket.charAt(deph % openBracket.length()) + subTrees.get(0).toString(deph + 1);
    for (int i=1;i<subTrees.size(); i++) result+=","+subTrees.get(i).toString(deph + 1);
    return result + closeBracket.charAt(deph % openBracket.length());
  }
